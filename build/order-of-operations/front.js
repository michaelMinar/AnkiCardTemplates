...
try {
  ...
  if (!answerElement) {
    ...
    if (!questionElement) {
      logDebug('Question element not found');
      return;
    }
    ...
    if (!numTermsEl || !includeExponentsEl) {
      logDebug('Configuration elements not found');
      return;
    }
    ...
    logDebug('Generating expression with ' + numTerms + ' terms, exponents: ' + includeExponents);
    ...
    logDebug('Expression displayed: ' + result.expression);
  }
